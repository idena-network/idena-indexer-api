...
func (s *httpServer) epochBlocksOld(w http.ResponseWriter, r *http.Request) {
	...
	startIndex, count, err := ReadOldPaginatorParams(vars)
	if err != nil {
		WriteErrorResponse(w, err, s.logger)
		return
	}
...
// Deprecated
func (s *httpServer) onlineIdentitiesOld(w http.ResponseWriter, r *http.Request) {
	startIndex, count, err := ReadOldPaginatorParams(mux.Vars(r))
	if err != nil {
		WriteErrorResponse(w, err, s.logger)
		return
	}
	resp, err := s.service.GetOnlineIdentitiesOld(startIndex, count)
	WriteResponse(w, resp, err, s.logger)
}
